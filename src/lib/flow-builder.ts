...
export const NodeRules = {
  ...
  validateMaxOutgoing(edges: EdgeLike[], max = 1): boolean {
    ...
    const hasMaxOutgoing = Array.from(bySource.values()).some(
      (occurrences) => occurrences > max
    );
    return hasMaxOutgoing;
  },

  validateMaxIncoming(edges: EdgeLike[], max = 1): boolean {
    const byTarget = new Map<string, number>();
    for (const e of edges || []) {
      let count = byTarget.get(e.target) || 0;
      count += 1;
      byTarget.set(e.target, count);
    }

    const hasMaxIncoming = Array.from(byTarget.values()).some(
      (occurrences) => occurrences > max
    );
    return hasMaxIncoming;
  },

  validateAllConnected(nodes: NodeLike[], edges: EdgeLike[]): boolean {
