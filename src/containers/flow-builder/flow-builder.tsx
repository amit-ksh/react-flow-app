...
import { NodeRules } from "@/lib/flow-builder";

export default function FlowBuilderWrapper() {
  const { editNodeId } = useFlowBuilder();
  const { getEdges, getNodes } = useReactFlow();
  const { setValue: setNodes } = useLocalStorageQuery<TextNodeData[]>(
    QUERY_KEYS.NODES,
    []
  );
  ...
  const handleSaveChanges = () => {
    ...
    const hasDisconnectedNode = NodeRules.validateAllConnected(nodes, edges);
